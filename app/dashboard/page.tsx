...
export default function DashboardPage() {
  ...
  const InventoryBreakdown = () => {
    const roastedValue = (roastedInventory as any[])?.reduce((acc, curr) => acc + (curr.quantityLbs * curr.costPerLb), 0) || 0;
    const pieData = [
    ...
    <div className="p-4 md:p-8 space-y-6 md:space-y-8 animate-in fade-in duration-500 relative">
                ...
                {burnDownData.slice(0, 5).map((item) => (
                   <div key={item._id} className="space-y-2">
                      ...
                      <div className="h-2.5 md:h-3 w-full bg-slate-100 rounded-full overflow-hidden">
                         <div className={`h-full rounded-full transition-all duration-1000 ${item.daysRemaining < 14 ? 'bg-red-500' : item.daysRemaining < 30 ? 'bg-amber-400' : 'bg-green-500'}`} style={{ width: `${Math.min((item.daysRemaining / 60) * 100, 100)}%` }} />
                      </div>
                   </div>
                ))}
             </div>
           ...
           <div className="bg-slate-900 text-white p-8 md:p-10 rounded-[2rem] flex flex-col justify-between relative overflow-hidden group shadow-2xl min-h-[400px]">
              ...
              <div className="relative z-10">
                 <div className="flex items-center gap-3 text-amber-500 mb-8 font-black uppercase tracking-[0.2em] text-[10px]">
                    <Lightbulb size={18} /> Virtual CFO Insight
                 </div>
                 <h3 className="text-xl md:text-2xl font-bold leading-snug mb-4">Focus on your margins.</h3>
      ...
      {activeMetric && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md overflow-y-auto">
          <div className="bg-white rounded-[2.5rem] w-full max-w-2xl p-6 md:p-10 shadow-2xl relative animate-in zoom-in-95 duration-200">
             ...
             <div className="mb-8">
                <h2 className="text-2xl font-black text-slate-900 capitalize">{activeMetric.replace('_', ' ')} Analysis</h2>
             </div>
