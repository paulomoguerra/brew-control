...
export function StatCard({ label, value, icon, trend, alert }: any) {
  return (
    <div className={`bg-white p-6 rounded-[2rem] border ${alert ? 'border-red-200 bg-red-50' : 'border-slate-200'} shadow-sm transition-all hover:shadow-lg`}>
      <div className="flex justify-between items-start mb-4">
        <div className={`p-3 rounded-xl border ${alert ? 'bg-white border-red-100' : 'bg-slate-50 border-slate-100'}`}>
          {icon}
        </div>
        {trend && (
          <span className={`text-[10px] font-black uppercase tracking-wider px-3 py-1 rounded-full ${alert ? 'bg-red-200 text-red-700' : 'bg-slate-100 text-slate-500'}`}>
            {trend}
          </span>
        )}
      </div>
